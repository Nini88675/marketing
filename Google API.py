...
# ============================================================
#  Vérification et conversion codec avec FFmpeg
# ============================================================
def check_ffmpeg():
...
def convert_to_h264(input_path):
    """Convertit la vidéo en H.264 (compatible OpenCV)"""
    output_path = input_path.replace(".mp4", "_h264.mp4")

    # Si déjà converti, on skip
    if os.path.exists(output_path):
        print(f"✅ Version H.264 existe déjà : {output_path}")
        return output_path

    print(f"🔄 Conversion en H.264 pour compatibilité OpenCV...")
    cmd = [
        "ffmpeg", "-i", input_path,
        "-c:v", "libx264",       # Codec H.264
        "-preset", "fast",       # Conversion rapide
        "-crf", "23",            # Qualité (18-28, 23 = bon compromis)
        "-c:a", "aac",           # Audio AAC
...
# ============================================================
# 4️ Téléchargement YouTube avec codec H.264 forcé
# ============================================================
def download_video(url):
    ydl_opts = {
        # ✅ FIX : Forcer H.264 (AVC) au lieu de H.265 (HEVC)
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if os.path.exists(video_path) and os.path.getsize(video_path) > 1_000_000:
            ...
            if not ret:
                print(f"⚠️ OpenCV ne peut pas lire la vidéo, conversion nécessaire...")
                if check_ffmpeg():
                    video_path = convert_to_h264(video_path)
                else:
                    print("❌ FFmpeg non installé. Installez-le avec: sudo apt-get install ffmpeg")
                    raise Exception("FFmpeg requis pour la conversion")

            return video_path
        else:
            raise Exception(f"[Erreur] Vidéo vide ou non téléchargée : {video_path}")

# ============================================================
# 5️Analyse Google Cloud Video Intelligence
# ============================================================
def analyze_video(video_path):
    ...
    print(f"⏳ Analyse de {video_path} ({file_size_mb:.1f} MB)...")

    operation = client.annotate_video(request={"features": features, "input_content": input_content})
    result = operation.result(timeout=600)
    print("✅ Analyse terminée")

    # 🔍 DEBUG : Vérifier ce que l'API retourne
    if DEBUG and result.annotation_results:
        ann = result.annotation_results[0]
        print(f"   📦 Objets détectés : {len(ann.object_annotations) if ann.object_annotations else 0}")
        print(f"   📝 Textes détectés : {len(ann.text_annotations) if ann.text_annotations else 0}")
        print(f"   🏷️ Labels détectés : {len(ann.segment_label_annotations) if ann.segment_label_annotations else 0}")

    return result.annotation_results[0]

# ============================================================
# 6Extraction de métriques marketing (AVEC FIX OPENCV)
# ============================================================
def compute_metrics(video_path, result):
    # ✅ FIX : Utiliser le backend FFmpeg de OpenCV
    ...
    try:
        ...
        if fps == 0 or total_frames == 0:
            print(f"⚠️ Vidéo invalide : fps={fps}, frames={total_frames}")
            return {
        ...
        if result.object_annotations:
            for obj in result.object_annotations:
                ...
                # 🔍 DEBUG : Afficher tous les objets détectés
                if DEBUG:
                    print(f"   🔎 Objet trouvé : '{desc}' ({len(obj.frames)} frames)")
        ...
        if DEBUG:
            print(f"   🍔 Burgers détectés : {burger_objects}")
            print(f"   ⏱️ Temps burger : {burger_time:.1f}s / {duration:.1f}s = {burger_ratio:.2%}")
        ...
        if result.text_annotations:
            for text in result.text_annotations:
                if DEBUG:
                    print(f"   📝 Texte trouvé : '{text.text}'")
        ...
        if DEBUG and promo_detected:
            print(f"   🎯 Mots promotionnels trouvés : {set(found_texts)}")
    ...
    "chili", "jalapeno", "mushroom", "olive", "cabbage", "carrot"]
                ...
                if any(veg in desc for veg in vegetable_keywords):
                    ...
                    if DEBUG:
                        print(f"   🥬 Végétal détecté : '{desc}'")

        if DEBUG:
            print(f"   🥬 Végétaux détectés : {vegetable_objects}")

        # --- Time_to_logo_appearance (NOUVEAU) ---
        logo_time = -1  # -1 = non détecté
                ...
                if any(logo in desc for logo in logo_keywords):
                    if obj.frames:
                        ...
                        if logo_time == -1 or first_frame_time < logo_time:
                            ...
                            if DEBUG:
                                print(f"   🏷️ Logo détecté : '{desc}' à {logo_time:.1f}s")
                ...
                if any(logo in text_lower for logo in logo_keywords):
                    if text.segments:
                        ...
                        if logo_time == -1 or first_segment_time < logo_time:
                            ...
                            if DEBUG:
                                print(f"   🏷️ Texte de logo détecté : '{text.text}' à {logo_time:.1f}s")

        # --- Color_intensity (saturation moyenne) ---
        # ✅ FIX : Échantillonnage plus intelligent
        num_samples = min(20, total_frames // fps)  # Max 20 échantillons ou 1 par seconde
        if num_samples == 0:
            num_samples = 1
        ...
        if DEBUG:
            print(f"   🎨 Extraction couleurs ({num_samples} échantillons, step={step})...")
        ...
        for i in range(num_samples):
            ...
            if not ret:
                if DEBUG:
                    print(f"   ⚠️ Frame {frame_idx} non lue (ret={ret})")
                continue

            if frame is None or frame.size == 0:
                if DEBUG:
                    print(f"   ⚠️ Frame {frame_idx} vide")
                continue
        ...
        if DEBUG:
            print(f"   ✅ Frames analysées : {frames_processed}/{num_samples}")
            if saturations:
                print(f"   📊 Saturations : min={min(saturations):.3f}, max={max(saturations):.3f}, mean={color_intensity:.3f}")
            else:
                print(f"   ⚠️ Aucune saturation calculée - problème de lecture vidéo")
...
# ============================================================
# 7️⃣ Boucle principale avec cache et gestion des erreurs
# ============================================================
results = {}

for url in tqdm(urls, desc="Analyse des vidéos"):
    print(f"\n{'='*60}")
    ...
    if os.path.exists(json_path):
        print(f"⚡ Résultat existant trouvé pour {video_id}, rechargement…")
        try:
            ...
            if not data:
                raise ValueError("JSON vide après parsing")

            # ✅ CORRECTION : Utiliser MessageToDict au lieu de to_dict()
            from google.protobuf.json_format import MessageToDict
            result_obj = videointelligence.VideoAnnotationResults()
            json_format.Parse(json.dumps(data), result_obj)
            res = result_obj
            print(f"✅ JSON rechargé avec succès")

        except (json.JSONDecodeError, ValueError) as e:
            print(f"⚠️ JSON corrompu ou vide ({e}), nouvelle analyse...")
            os.remove(json_path)
            ...
            try:
                ...
                print(f"💾 Nouveau JSON sauvegardé : {json_path} ({file_size} bytes)")

            except Exception as e2:
                print(f"[Erreur] Analyse échouée pour {video_id} : {e2}")
                continue
    else:
        try:
            res = analyze_video(video_path)
            ...
            try:
                ...
                file_size = os.path.getsize(json_path)
                print(f"💾 Résultat sauvegardé : {json_path} ({file_size} bytes)")

            except Exception as save_error:
                print(f"⚠️ Erreur lors de la sauvegarde JSON : {save_error}")

        except Exception as e:
            print(f"[Erreur] Analyse échouée pour {video_id} : {e}")
            continue
    ...
    try:
        ...
        print(f"[Erreur] Calcul de métriques échoué pour {video_id} : {e}")
        import traceback
        traceback.print_exc()
        continue

# ============================================================
# 8️⃣ Résultats et visualisation
# ============================================================
print("\n" + "="*60)
...
if results:
    ...
    plt.xticks(x, [f"Vidéo {i+1}" for i in range(len(results))], rotation=45)
    plt.ylabel("Burger Screen Ratio")
    plt.title("Importance visuelle du burger par vidéo")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/burger_comparison.png", dpi=150)
    print("\n📈 Graphique sauvegardé : results/burger_comparison.png")
    plt.show()
else:
    print("⚠️ Aucun résultat à afficher.")

print(f"\n✅ Analyse terminée ! {len(results)}/{len(urls)} vidéos traitées.")
